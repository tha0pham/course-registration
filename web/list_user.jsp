...
<html lang="en">
    ...
    <%
        ...
        int total = userDAO.findAllUsers().size();
        int numPerPage = 5;
        int pageNumber = 0;
        if (request.getParameter("pageNum") != null) {
            pageNumber = Integer.parseInt(request.getParameter("pageNum"));
        }
        List<User> users = userDAO.getUsers(numPerPage, (pageNumber * numPerPage));
            ...
            <div class="container">
                ...
                <section id="content">
                    <div class="inside">        
                        ...
                        <form method="post" action="EditUserServlet" name="fmUser">
                            <table class="myListTable">
                                ...
                                <tr>
                                    <td><%=i%></td>
                                    <td><a href="edit_user.jsp?id=<%=each.getId()%>"><%=each.getUsername()%></a></td>
                                    <td><%=each.getLastName()%></td>
                                    <td><%=each.getFirstName()%></td>

                                    <td><a href="DeleteUserServlet?id=<%=each.getId()%>">Delete</a></td>
                                </tr>
                                <%
                                        }
                                    }
                            ...
                            <%
                                ...
                                if (pageNumber != 0) {
                                    out.print("<a href='list_user.jsp?pageNum=" + (pageNumber - 1)
                                            + "'>Previous</a> ");
                                }
                                out.print("Page ");
                                for (int j = 0; j < totalPages; j++) {
                                    ...
                                    if (j != pageNumber) {
                                        out.print("<a href='list_user.jsp?pageNum=" + j
                                                + "'>" + (j + 1) + "</a> ");
                                    } else {
                                        out.print((j + 1) + " ");
                                    }
                            ...
                            %>
                            <p>
                                <input type="button" name="btnToAdd" value="Add New User" onclick="location.href = 'edit_user.jsp'"/>
                            </p>
                        </form>
                        <p>${message}</p>
