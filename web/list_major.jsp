...
<!--[if lt IE 9]>
 ...
 <%
        ...
        if (keyword != null) {
            majors = majorDAO.search(keyword);
        }
    %>
...
esse quam nihil molestiae. </li>
 ...
 <form name="fmMajor" method="get">
            <table class="myListTable">
                ...
                <tr>
                    <td><%=i%></td>
                    <td><a href="edit_major.jsp?id=<%=each.getId()%>"><%=each.getName()%></a></td>
                    <td><%=each.getDescription()%></td>
                    <td><%=each.getFaculty().getName()%></td>
                    <td><a href="DeleteMajorServlet?id=<%=each.getId()%>">Delete</a></td>
                </tr>
                <%
                        }
                    }
            ...
            <div>
                <input type="button" name="btnToAdd" value="Add New Major" onclick="location.href = 'edit_major.jsp'" />
            </div>
        </form>
        <p>${message}</p>
