...
<html lang="en">
    ...
    <%
        ...
        int total = instructorDAO.findAllInstructors().size();
        int numPerPage = 5;
        int pageNumber = 0;
        if (request.getParameter("pageNum") != null) {
            pageNumber = Integer.parseInt(request.getParameter("pageNum"));
        }
        List<Instructor> instructors = instructorDAO.getInstructors(numPerPage, (pageNumber * numPerPage));

        String keyword = request.getParameter("keyword");
        if (keyword != null) {
            instructors = instructorDAO.search(keyword);
        }
    %>
            ...
            <div class="container">
                ...
                <section id="content">
                    <div class="inside">        
                        ...
                        <form method="post" action="AddInstructorServlet" name="fmInstructor">
                            <table class="myListTable">
                                ...
                                <tr>
                                    <td><%=i%></td>
                                    <td><a href="edit_instructor.jsp?id=<%=each.getId()%>"><%=each.getCode()%></a></td>
                                    <td><%=each.getLastName()%></td>
                                    <td><%=each.getFirstName()%></td>
                                    <td><a href="DeleteInstructorServlet?id=<%=each.getId()%>">Delete</a></td>
                                </tr>
                                <%
                                        }
                                    }
                            ...
                            <%
                                ...
                                if (pageNumber != 0) {
                                    out.print("<a href='list_instructor.jsp?pageNum=" + (pageNumber - 1)
                                            + "'>Previous</a> ");
                                }
                                out.print("Page ");
                                for (int j = 0; j < totalPages; j++) {
                                    ...
                                    if (j != pageNumber) {
                                        out.print("<a href='list_instructor.jsp?pageNum=" + j
                                                + "'>" + (j + 1) + "</a> ");
                                    } else {
                                        out.print((j + 1) + " ");
                                    }
                            ...
                            %>
                            <div>
                            <input type="button" name="btnToAdd" value="Add New Instructor" onclick="location.href = 'edit_instructor.jsp'"/>
                            </div>
                        </form>
                        <p>${message}</p>
